...
class TypeEntityTransformer(CSTTransformer):
    ...
    def _render_alias(self, name: str, alias: AnyDict):
        ...
        params_description.append(f":return: instance of method :code:`{alias['method']}`")
        fill_names = ", ".join(f":code:`{name}`" for name in alias["fill"].keys())
        description_lines = [
            f"Shortcut for method **{alias['method']}** will automatically fill {fill_names} method attributes",
