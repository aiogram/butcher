...
@click.argument("names", nargs=-1, type=EntityNameType("types"))
...
@click.argument("names", nargs=-1, type=EntityNameType("methods"))
@click.option("--diff", is_flag=True, default=False)
@pass_config
@pass_registry
...
@click.argument("names", nargs=-1, type=EntityNameType("enums"))
@click.option("--diff", is_flag=True, default=False)
@pass_config
@pass_registry
...
def _apply(
    ...
    with alive_bar(
        ...
        for name in names:
            ...
            if diff:
                show_diff(code_path, code, new_code)
            else:
...
def show_diff(path: Path, a: str, b: str):
